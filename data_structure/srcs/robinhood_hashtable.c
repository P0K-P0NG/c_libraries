...
void RobinHashTableAddBucket(
    RobinHTBucket **buckets, RobinHTBucket *new_bucket,
    unsigned long i, unsigned long max_count);

RobinHashTable *RobinHashTableCreate(
    unsigned long bucket_count, float max_load_prop)
{
    RobinHashTable *new_table = calloc(1, sizeof(RobinHashTable));
    if (new_table == NULL)
    {
        return NULL;
    }
    ...
    new_table->buckets = calloc(bucket_count, sizeof(RobinHTBucket *));
...
void RobinHashTableClear(
    RobinHashTable **table, void (*free_key)(void *), void (*free_data)(void *))
{
...
int RobinHashTableAdd(
    RobinHashTable *table, void *key,
    void *data, unsigned long (*hash_func)(const void *))
{
    ...
    else if ((float)table->count.used / table->count.max >= table->max_load || table->count.used >= table->count.max)
    {
    ...
    RobinHTBucket *new_bucket = calloc(1, sizeof(RobinHTBucket));
    if (new_bucket == NULL)
    {
        return 0;
    }
...
void RobinHashTableAddBucket(
    ...
    unsigned long i, unsigned long max_count)
{
    RobinHTBucket *curr_bucket = new_bucket;
    for (; buckets[i] != NULL; i = (i + 1) % max_count)
    {
        if (curr_bucket->psl > buckets[i]->psl)
        {
            swap(&curr_bucket, &buckets[i], sizeof(RobinHTBucket *));
        }
        curr_bucket->psl += 1;
    }
...
void *RobinHashTableFind(
    RobinHashTable *table,
    void *key,
    unsigned long (*hash_func)(const void *),
    int (*comp_key)(const void *, const void *))
{
    ...
    unsigned long (*hash_func)(const void *))
{
    ...
    RobinHTBucket **new_buckets = calloc(new_count, sizeof(RobinHTBucket *));
    if (new_buckets == NULL)
    {
        return 0;
    }
    unsigned long rehash_count = 0;
    for (unsigned long i = 0; i < table->count.max && rehash_count <= table->count.used; i++)
    {
        if (table->buckets[i] != NULL)
        {
            unsigned long new_i = hash_func(table->buckets[i]->key) % new_count;
            RobinHashTableAddBucket(
