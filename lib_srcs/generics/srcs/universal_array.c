...
void ArraySetItem(struct Array arr, unsigned int idx, void *item)
{
...
int ArrayGetIdx(struct Array arr, const void *item,
...
void ArrayInsertSort(struct Array *arr,
                     int (*comp_func)(const void *, const void *))
{
...
int array_to_int(struct Array arr)
{
    int result = 0;
    for (int i = 0; i < arr.len; i++) {
        result *= 10;
